...
namespace AnimeFavoritenManager.Daten
{
    internal class AnimeDatenVerwalter
    {
        ...
        public async Task AnimeVonApiLadenAsync()
        {
            ...
            {
                // 1. URL festlegen
                string url = "https://api.jikan.moe/v4/top/anime?type=tv&sfw=true";

                // 2. HTTP-Request schicken (await) und Response frühzeitig freigeben
                using HttpResponseMessage response = await httpClient.GetAsync(url);

                // 3. Status prüfen
                if (!response.IsSuccessStatusCode)
                {
                    FarbAusgabe.SchreibeFehler($"Fehler beim Abrufen der Anime-Daten. Statuscode: {response.StatusCode}");
                    return;
                }
                ...
                // 5. JSON deserialisieren (Case-Insensitive)
                JsonSerializerOptions options = new JsonSerializerOptions
                {
                    PropertyNameCaseInsensitive = true
                };
                JikanAnimeResponse? apiAntwort = JsonSerializer.Deserialize<JikanAnimeResponse>(json, options);

                if (apiAntwort == null || apiAntwort.Data == null || apiAntwort.Data.Count == 0)
                {
                    FarbAusgabe.SchreibeFehler("Es konnten keine Daten aus der API geladen werden.");
                    return;
                }
                ...
                foreach (JikanAnime eintrag in apiAntwort.Data)
                {
                ...
                FarbAusgabe.SchreibeErfolg($"Es wurden {animeListe.Count} Anime aus der API geladen.");
            }
            catch (Exception ex)
            {
                // 7. Fehler für den Benutzer ausgeben
                FarbAusgabe.SchreibeFehler($"Beim Abrufen ist ein Fehler aufgetreten: {ex.Message}");
            }
        }
